...
define(["require", "exports", "assert", "vs/base/common/uri", "vs/workbench/api/node/extHostTypes", "vs/base/common/platform"], function (require, exports, assert, uri_1, types, platform_1) {
    ...
    suite('ExtHostTypes', function () {
        test('URI, toJSON', function () {
            let uri = uri_1.URI.parse('file:///path/test.file');
        ...
        test('WorkspaceEdit', () => {
            let a = uri_1.URI.file('a.ts');
            let b = uri_1.URI.file('b.ts');
        ...
        test('WorkspaceEdit - keep order of text and file changes', function () {
            ...
            function isFileChange(thing) {
                const [f, s] = thing;
                return uri_1.URI.isUri(f) && uri_1.URI.isUri(s);
            }
            function isTextChange(thing) {
                const [f, s] = thing;
                return uri_1.URI.isUri(f) && Array.isArray(s);
            }
        ...
        test('WorkspaceEdit - two edits for one resource', function () {
            ...
            assert.equal(first[1][0].newText, 'Hello');
            assert.equal(second[1][0].newText, 'Foo');
        });
        test('DocumentLink', () => {
