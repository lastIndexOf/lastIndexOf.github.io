    ...
    class ReplModel {
        ...
        appendToRepl(data, sev, source) {
            ...
            if (typeof data === 'string') {
                const previousElement = this.replElements.length && this.replElements[this.replElements.length - 1];
                const toAdd = data.split('\n').map((line, index) => new debugModel_1.SimpleReplElement(`topReplElement:${topReplElementCounter++}`, line, sev, index === 0 ? source : undefined));
                if (previousElement && previousElement.value === '') {
                    // remove potential empty lines between different repl types
                    this.replElements.pop();
                }
                else if (previousElement instanceof debugModel_1.SimpleReplElement && sev === previousElement.severity && toAdd.length && toAdd[0].sourceData === previousElement.sourceData) {
