 ...
 *--------------------------------------------------------------------------------------------*/
define(["require", "exports", "vs/nls", "vs/base/common/severity", "vs/platform/extensions/common/extensions"], function (require, exports, nls, severity_1, extensions_1) {
    "use strict";
    ...
    class ExtensionsActivator {
        ...
        _handleActivateRequest(currentExtensionId, greenExtensions, redExtensions) {
            ...
            for (let j = 0, lenJ = depIds.length; j < lenJ; j++) {
                ...
                if (dep && dep.activationFailed) {
                    ...
                    this._host.showMessage(severity_1.default.Error, nls.localize('failedDep1', "Cannot activate extension '{0}' because it depends on extension '{1}', which failed to activate.", currentExtension.displayName || currentExtension.identifier.value, depId));
                ...
                this._host.showMessage(severity_1.default.Error, nls.localize('unknownDep', "Cannot activate extension '{0}' because it depends on extension '{1}', which is not installed or disabled. Please install or enable '{1}' and reload the window.", currentExtension.displayName || currentExtension.identifier.value, depId));
        ...
        _activateExtension(extensionId, reason) {
            ...
            const newlyActivatingExtension = this._host.actualActivateExtension(extensionId, reason).then(undefined, (err) => {
                this._host.showMessage(severity_1.default.Error, nls.localize('activationError', "Activating extension '{0}' failed: {1}.", extensionId.value, err.message));
