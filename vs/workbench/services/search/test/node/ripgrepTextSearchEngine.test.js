...
define(["require", "exports", "assert", "vs/base/common/resources", "vs/base/common/uri", "vs/workbench/services/search/node/ripgrepSearchUtils", "vs/workbench/services/search/node/ripgrepTextSearchEngine"], function (require, exports, assert, resources_1, uri_1, ripgrepSearchUtils_1, ripgrepTextSearchEngine_1) {
    "use strict";
    ...
    suite('RipgrepTextSearchEngine', () => {
        ...
        test('fixRegexEndingPattern', () => {
            function testFixRegexEndingPattern([input, expectedResult]) {
                assert.equal(ripgrepTextSearchEngine_1.fixRegexEndingPattern(input), expectedResult);
            }
            [
                ['foo', 'foo'],
                ['', ''],
        ...
        test('fixRegexCRMatchingWhitespaceClass', () => {
            function testFixRegexCRMatchingWhitespaceClass([inputReg, isMultiline, testStr, shouldMatch]) {
                const fixed = ripgrepTextSearchEngine_1.fixRegexCRMatchingWhitespaceClass(inputReg, isMultiline);
        ...
        test('fixRegexCRMatchingNonWordClass', () => {
            function testRegexCRMatchingNonWordClass([inputReg, isMultiline, testStr, shouldMatch]) {
                const fixed = ripgrepTextSearchEngine_1.fixRegexCRMatchingNonWordClass(inputReg, isMultiline);
                const reg = new RegExp(fixed);
                assert.equal(reg.test(testStr), shouldMatch, `${inputReg} => ${reg}, ${testStr}, ${shouldMatch}`);
            }
            [
            ...
            test('single result', () => {
                testParser([
                ...
                ], [
                    {
                        preview: {
                            text: 'foobar',
                            matches: [new ripgrepSearchUtils_1.Range(0, 3, 0, 6)]
                        },
                        uri: resources_1.joinPath(TEST_FOLDER, 'file1.js'),
                        ranges: [new ripgrepSearchUtils_1.Range(3, 3, 3, 6)]
                    }
                ]);
            });
