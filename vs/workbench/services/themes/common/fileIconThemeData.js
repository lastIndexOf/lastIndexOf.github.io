        ...
        for (let defId in selectorByDefinitionId) {
            ...
            if (definition) {
                if (definition.iconPath) {
                    let cssRule = `${selectors.join(', ')} { content: ' '; background-image: url("${resolvePath(definition.iconPath)}"); }`;
                    if (process.env.isBrowser) {
                        cssRule = cssRule.replace(/(url\(['"]?)file:\/\//g, '$1');
                    }
