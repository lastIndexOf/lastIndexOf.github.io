    ...
    let PreferencesService = class PreferencesService extends lifecycle_1.Disposable {
        ...
        resolveModel(uri) {
            ...
            if (this.defaultSettingsRawResource.toString() === uri.toString()) {
                const defaultRawSettingsEditorModel = this.instantiationService.createInstance(preferencesModels_1.DefaultRawSettingsEditorModel, this.getDefaultSettings(1 /* USER */));
                const languageSelection = this.modeService.create('jsonc');
        ...
        createPreferencesEditorModel(uri) {
            ...
            const workspaceSettingsUri = this.getEditableSettingsURI(2 /* WORKSPACE */);
        ...
        openFolderSettings(folder, jsonEditor, options, group) {
            jsonEditor = typeof jsonEditor === 'undefined' ?
                this.configurationService.getValue('workbench.settings.editor') === 'json' :
                jsonEditor;
            const folderSettingsUri = this.getEditableSettingsURI(3 /* WORKSPACE_FOLDER */, folder);
            if (jsonEditor) {
                if (folderSettingsUri) {
                    return this.openOrSwitchSettings(3 /* WORKSPACE_FOLDER */, folderSettingsUri, options, group);
                }
                return Promise.reject(`Invalid folder URI - ${folder.toString()}`);
            }
            return this.openOrSwitchSettings2(3 /* WORKSPACE_FOLDER */, folder, options, group);
        }
        ...
        openGlobalKeybindingSettings(textual) {
            ...
            */
            ...
            if (textual) {
                ...
                const editableKeybindings = uri_1.URI.file(this.environmentService.appKeybindingsPath);
        ...
        doOpenSettings2(target, folderUri, options, group) {
            const input = this.settingsEditor2Input;
            const settingsOptions = Object.assign({}, options, { target,
                folderUri });
        ...
        createEditableSettingsEditorModel(configurationTarget, resource) {
