...
define(["require", "exports", "sinon", "assert", "os", "vs/base/common/path", "fs", "vs/base/common/json", "vs/platform/registry/common/platform", "vs/platform/environment/common/environment", "vs/platform/environment/node/argv", "vs/platform/workspace/common/workspace", "vs/platform/environment/node/environmentService", "vs/base/node/extfs", "vs/workbench/test/workbenchTestServices", "vs/platform/notification/test/common/testNotificationService", "vs/base/common/uuid", "vs/platform/configuration/common/configurationRegistry", "vs/workbench/services/configuration/node/configurationService", "vs/workbench/services/files/node/fileService", "vs/workbench/services/configuration/node/configurationEditingService", "vs/platform/files/common/files", "vs/workbench/services/configuration/common/configuration", "vs/platform/configuration/common/configuration", "vs/workbench/services/textfile/common/textfiles", "vs/editor/common/services/resolverService", "vs/workbench/services/textmodelResolver/common/textModelResolverService", "vs/platform/configuration/test/common/testConfigurationService", "vs/base/node/pfs", "vs/platform/notification/common/notification", "vs/platform/commands/common/commands", "vs/workbench/services/commands/common/commandService", "vs/base/common/uri", "crypto"], function (require, exports, sinon, assert, os, path, fs, json, platform_1, environment_1, argv_1, workspace_1, environmentService_1, extfs, workbenchTestServices_1, testNotificationService_1, uuid, configurationRegistry_1, configurationService_1, fileService_1, configurationEditingService_1, files_1, configuration_1, configuration_2, textfiles_1, resolverService_1, textModelResolverService_1, testConfigurationService_1, pfs_1, notification_1, commands_1, commandService_1, uri_1, crypto_1) {
    ...
    suite('ConfigurationEditingService', () => {
        ...
        test('errors cases - invalid key', () => {
            return testObject.writeConfiguration(2 /* WORKSPACE */, { key: 'unknown.key', value: 'value' })
        ...
        test('errors cases - invalid target', () => {
            return testObject.writeConfiguration(1 /* USER */, { key: 'tasks.something', value: 'value' })
                .then(() => assert.fail('Should fail with ERROR_INVALID_TARGET'), (error) => assert.equal(error.code, 3 /* ERROR_INVALID_USER_TARGET */));
        });
        test('errors cases - no workspace', () => {
            return setUpServices(true)
                .then(() => testObject.writeConfiguration(2 /* WORKSPACE */, { key: 'configurationEditing.service.testSetting', value: 'value' }))
                .then(() => assert.fail('Should fail with ERROR_NO_WORKSPACE_OPENED'), (error) => assert.equal(error.code, 6 /* ERROR_NO_WORKSPACE_OPENED */));
        });
        test('errors cases - invalid configuration', () => {
            fs.writeFileSync(globalSettingsFile, ',,,,,,,,,,,,,,');
            return testObject.writeConfiguration(1 /* USER */, { key: 'configurationEditing.service.testSetting', value: 'value' })
                .then(() => assert.fail('Should fail with ERROR_INVALID_CONFIGURATION'), (error) => assert.equal(error.code, 8 /* ERROR_INVALID_CONFIGURATION */));
        });
        test('errors cases - dirty', () => {
            instantiationService.stub(textfiles_1.ITextFileService, 'isDirty', true);
            return testObject.writeConfiguration(1 /* USER */, { key: 'configurationEditing.service.testSetting', value: 'value' })
                .then(() => assert.fail('Should fail with ERROR_CONFIGURATION_FILE_DIRTY error.'), (error) => assert.equal(error.code, 7 /* ERROR_CONFIGURATION_FILE_DIRTY */));
        });
        test('dirty error is not thrown if not asked to save', () => {
            instantiationService.stub(textfiles_1.ITextFileService, 'isDirty', true);
            return testObject.writeConfiguration(1 /* USER */, { key: 'configurationEditing.service.testSetting', value: 'value' }, { donotSave: true })
                .then(() => null, error => assert.fail('Should not fail.'));
        });
        test('do not notify error', () => {
            instantiationService.stub(textfiles_1.ITextFileService, 'isDirty', true);
            const target = sinon.stub();
            instantiationService.stub(notification_1.INotificationService, { prompt: target, _serviceBrand: null, notify: null, error: null, info: null, warn: null });
            return testObject.writeConfiguration(1 /* USER */, { key: 'configurationEditing.service.testSetting', value: 'value' }, { donotNotifyError: true })
                .then(() => assert.fail('Should fail with ERROR_CONFIGURATION_FILE_DIRTY error.'), (error) => {
                assert.equal(false, target.calledOnce);
                assert.equal(error.code, 7 /* ERROR_CONFIGURATION_FILE_DIRTY */);
            });
        });
        ...
        test('remove an existing setting - existing file', () => {
            ...
            return testObject.writeConfiguration(1 /* USER */, { key: 'configurationEditing.service.testSetting', value: undefined })
                .then(() => {
                const contents = fs.readFileSync(globalSettingsFile).toString('utf8');
                const parsed = json.parse(contents);
        ...
        test('write workspace standalone setting - empty file', () => {
            return testObject.writeConfiguration(2 /* WORKSPACE */, { key: 'tasks.service.testSetting', value: 'value' })
                .then(() => {
        ...
        test('write workspace standalone setting - existing file', () => {
            ...
            return testObject.writeConfiguration(2 /* WORKSPACE */, { key: 'launch.service.testSetting', value: 'value' })
                .then(() => {
                const contents = fs.readFileSync(target).toString('utf8');
                const parsed = json.parse(contents);
                assert.equal(parsed['service.testSetting'], 'value');
        ...
        test('write workspace standalone setting - empty file - full JSON', () => {
            return testObject.writeConfiguration(2 /* WORKSPACE */, { key: 'tasks', value: { 'version': '1.0.0', tasks: [{ 'taskName': 'myTask' }] } })
                .then(() => {
                const target = path.join(workspaceDir, configuration_1.WORKSPACE_STANDALONE_CONFIGURATIONS['tasks']);
                const contents = fs.readFileSync(target).toString('utf8');
                const parsed = json.parse(contents);
        ...
        test('write workspace standalone setting should replace complete file', () => {
            ...
            return testObject.writeConfiguration(2 /* WORKSPACE */, { key: 'tasks', value: { 'version': '1.0.0', tasks: [{ 'taskName': 'myTask1' }] } })
                .then(() => {
