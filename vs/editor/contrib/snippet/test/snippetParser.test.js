define(["require", "exports", "assert", "vs/editor/contrib/snippet/snippetParser"], function (require, exports, assert, snippetParser_1) {
    ...
    suite('SnippetParser', () => {
        ...
        test('Parser, TM text', () => {
            ...
            let [, placeholder] = new snippetParser_1.SnippetParser().parse('foo${1:bar\\}${2:foo}}').children;
        ...
        test('Parser, placeholder with choice', () => {
            ...
            const expected = [snippetParser_1.Placeholder, snippetParser_1.Text, snippetParser_1.Text, snippetParser_1.Text];
            snippet.walk(marker => {
        ...
        test('Parser, transform example', () => {
            let { children } = new snippetParser_1.SnippetParser().parse('${1:name} : ${2:type}${3/\\s:=(.*)/${1:+ :=}${1}/};\n$0');
            //${1:name}
        ...
        test('backspace esapce in TM only, #16212', () => {
            const actual = new snippetParser_1.SnippetParser().text('Foo \\\\${abc}bar');
            assert.equal(actual, 'Foo \\bar');
        });
        test('colon as variable/placeholder value, #16717', () => {
            let actual = new snippetParser_1.SnippetParser().text('${TM_SELECTED_TEXT:foo:bar}');
            assert.equal(actual, 'foo:bar');
        ...
        test('TextmateSnippet#replace 1/2', function () {
            let snippet = new snippetParser_1.SnippetParser().parse('aaa${1:bbb${2:ccc}}$0', true);
            assert.equal(snippet.placeholders.length, 3);
            ...
            let nested = new snippetParser_1.SnippetParser().parse('ddd$1eee$0', true);
        ...
        test('TextmateSnippet#replace 2/2', function () {
            ...
            const [, second] = snippet.placeholders;
            assert.equal(second.index, '2');
            let nested = new snippetParser_1.SnippetParser().parse('dddeee$0', true);
            snippet.replace(second, nested.children);
            assert.equal(snippet.toString(), 'aaabbbdddeee');
            assert.equal(snippet.placeholders.length, 3);
        });
        ...
        test('Transform -> FormatString#resolve', function () {
            ...
            assert.equal(new snippetParser_1.FormatString(1, 'pascalcase').resolve('bar-foo'), 'BarFoo');
        ...
        test('Variable transformation doesn\'t work if undefined variables are used in the same snippet #51769', function () {
            let transform = new snippetParser_1.Transform();
        ...
        test('Snippet parser freeze #53144', function () {
            let snippet = new snippetParser_1.SnippetParser().parse('${1/(void$)|(.+)/${1:?-\treturn nil;}/}');
            assertMarker(snippet, snippetParser_1.Placeholder);
        });
        ...
        test('Mirroring sequence of nested placeholders not selected properly on backjumping #58736', function () {
            let snippet = new snippetParser_1.SnippetParser().parse('${3:nest1 ${1:nest2 ${2:nest3}}} $3');
        ...
        test('Backspace can\'t be escaped in snippet variable transforms #65412', function () {
            let snippet = new snippetParser_1.SnippetParser().parse('namespace ${TM_DIRECTORY/[\\/]/\\\\/g};');
