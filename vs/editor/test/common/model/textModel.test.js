...
define(["require", "exports", "assert", "vs/base/common/strings", "vs/editor/common/core/position", "vs/editor/common/core/range", "vs/editor/common/model/textModel", "vs/editor/test/common/editorTestUtils"], function (require, exports, assert, strings_1, position_1, range_1, textModel_1, editorTestUtils_1) {
    ...
    function testGuessIndentation(defaultInsertSpaces, defaultTabSize, expectedInsertSpaces, expectedTabSize, text, msg) {
        let m = editorTestUtils_1.createTextModel(text.join('\n'), {
    ...
    suite('TextModelData.fromString', () => {
        function testTextModelDataFromString(text, expected) {
            const textBuffer = textModel_1.createTextBuffer(text, textModel_1.TextModel.DEFAULT_CREATION_OPTIONS.defaultEOL);
            let actual = {
    ...
    suite('Editor Model - TextModel', () => {
        test('getValueLengthInRange', () => {
            let m = textModel_1.TextModel.createFromString('My First Line\r\nMy Second Line\r\nMy Third Line');
        ...
        test('validatePosition', () => {
            let m = textModel_1.TextModel.createFromString('line one\nline two');
        ...
        test('validatePosition around high-low surrogate pairs 1', () => {
            let m = textModel_1.TextModel.createFromString('a📚b');
            assert.deepEqual(m.validatePosition(new position_1.Position(0, 0)), new position_1.Position(1, 1));
            assert.deepEqual(m.validatePosition(new position_1.Position(0, 1)), new position_1.Position(1, 1));
        ...
        test('validatePosition handle NaN.', () => {
            let m = textModel_1.TextModel.createFromString('line one\nline two');
            assert.deepEqual(m.validatePosition(new position_1.Position(NaN, 1)), new position_1.Position(1, 1));
            assert.deepEqual(m.validatePosition(new position_1.Position(1, NaN)), new position_1.Position(1, 1));
            assert.deepEqual(m.validatePosition(new position_1.Position(NaN, NaN)), new position_1.Position(1, 1));
            assert.deepEqual(m.validatePosition(new position_1.Position(2, NaN)), new position_1.Position(2, 1));
            assert.deepEqual(m.validatePosition(new position_1.Position(NaN, 3)), new position_1.Position(1, 3));
        });
        ...
        test('#50471. getValueInRange with invalid range', () => {
            let m = textModel_1.TextModel.createFromString('My First Line\r\nMy Second Line\r\nMy Third Line');
            assert.equal(m.getValueInRange(new range_1.Range(1, NaN, 1, 3)), 'My');
            assert.equal(m.getValueInRange(new range_1.Range(NaN, NaN, NaN, NaN)), '');
        });
    });
    suite('TextModel.mightContainRTL', () => {
        test('nope', () => {
            let model = textModel_1.TextModel.createFromString('hello world!');
            assert.equal(model.mightContainRTL(), false);
        });
        test('yes', () => {
            let model = textModel_1.TextModel.createFromString('Hello,\nזוהי עובדה מבוססת שדעתו');
    ...
    suite('TextModel.createSnapshot', () => {
        ...
        test('file with BOM', () => {
            let model = textModel_1.TextModel.createFromString(strings_1.UTF8_BOM_CHARACTER + 'Hello');
        ...
        test('regular file', () => {
            let model = textModel_1.TextModel.createFromString('My First Line\n\t\tMy Second Line\n    Third Line\n\n1');
            let snapshot = model.createSnapshot();
