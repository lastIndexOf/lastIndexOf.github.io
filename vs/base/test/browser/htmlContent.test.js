define(["require", "exports", "assert", "vs/base/common/marked/marked", "vs/base/browser/htmlContentRenderer"], function (require, exports, assert, marked, htmlContentRenderer_1) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    suite('HtmlContent', () => {
        test('render simple element', () => {
            let result = htmlContentRenderer_1.renderText('testing');
        ...
        test('simple formatting', () => {
            let result = htmlContentRenderer_1.renderFormattedText('**bold**');
        ...
        test('no formatting', () => {
            let result = htmlContentRenderer_1.renderFormattedText('this is just a string');
            assert.strictEqual(result.innerHTML, 'this is just a string');
        });
        test('preserve newlines', () => {
            let result = htmlContentRenderer_1.renderFormattedText('line one\nline two');
            assert.strictEqual(result.innerHTML, 'line one<br>line two');
        });
        test('action', () => {
            let callbackCalled = false;
            let result = htmlContentRenderer_1.renderFormattedText('[[action]]', {
                actionHandler: {
                    callback(content) {
        ...
        test('fancy action', () => {
            let callbackCalled = false;
            let result = htmlContentRenderer_1.renderFormattedText('__**[[action]]**__', {
                actionHandler: {
                    callback(content) {
                        assert.strictEqual(content, '0');
                        callbackCalled = true;
        ...
        test('image rendering conforms to default', () => {
            ...
            const result = htmlContentRenderer_1.renderMarkdown(markdown);
        ...
        test('image width from title params', () => {
            let result = htmlContentRenderer_1.renderMarkdown({ value: `![image](someimageurl|width=100 'caption')` });
            assert.strictEqual(result.innerHTML, `<p><img src="someimageurl" alt="image" title="caption" width="100"></p>`);
        });
        test('image height from title params', () => {
            let result = htmlContentRenderer_1.renderMarkdown({ value: `![image](someimageurl|height=100 'caption')` });
            assert.strictEqual(result.innerHTML, `<p><img src="someimageurl" alt="image" title="caption" height="100"></p>`);
        });
        test('image width and height from title params', () => {
            let result = htmlContentRenderer_1.renderMarkdown({ value: `![image](someimageurl|height=200,width=100 'caption')` });
