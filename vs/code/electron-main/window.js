...
define(["require", "exports", "vs/base/common/path", "vs/base/common/objects", "vs/nls", "vs/base/common/uri", "vs/platform/state/common/state", "electron", "vs/platform/environment/common/environment", "vs/platform/log/common/log", "vs/platform/configuration/common/configuration", "vs/platform/environment/node/argv", "vs/platform/product/node/product", "vs/platform/windows/common/windows", "vs/base/common/lifecycle", "vs/base/common/platform", "vs/platform/workspaces/common/workspaces", "vs/platform/backup/common/backup", "vs/base/common/performance", "vs/code/electron-main/theme", "vs/base/common/async", "vs/base/common/types"], function (require, exports, path, objects, nls, uri_1, state_1, electron_1, environment_1, log_1, configuration_1, argv_1, product_1, windows_1, lifecycle_1, platform_1, workspaces_1, backup_1, perf, theme_1, async_1, types_1) {
    "use strict";
    ...
    let CodeWindow = class CodeWindow extends lifecycle_1.Disposable {
        constructor(config, logService, environmentService, configurationService, stateService, workspacesMainService, backupMainService) {
            super();
        ...
        createBrowserWindow(config) {
            ...
            const options = {
                ...
                backgroundColor: this.nodeless ? undefined : theme_1.getBackgroundColor(this.stateService),
        ...
        registerNavigationListenerOn(command, back, forward, acrossEditors) {
            this._win.on(command /* | 'app-command' */, (e, cmd) => {
                if (!this.isReady) {
                    return; // window must be ready
                }
                if (cmd === back) {
                    this.send('vscode:runAction', { id: acrossEditors ? 'workbench.action.openPreviousRecentlyUsedEditor' : 'workbench.action.navigateBack', from: 'mouse' });
                }
                else if (cmd === forward) {
                    this.send('vscode:runAction', { id: acrossEditors ? 'workbench.action.openNextRecentlyUsedEditor' : 'workbench.action.navigateForward', from: 'mouse' });
                }
            });
        }
        ...
        restoreWindowState(state) {
            if (state) {
                try {
                    state = types_1.withNullAsUndefined(this.validateWindowState(state));
                }
                catch (err) {
        ...
        validateWindowState(state) {
            ...
            if (display && // we have a display matching the desired bounds
                ...
                if (state.mode === 0 /* Maximized */) {
                    const defaults = exports.defaultWindowState(0 /* Maximized */); // when maximized, make sure we have good values when the user restores the window
