...
define(["require", "exports", "fs", "vs/base/common/path", "vs/nls", "vs/base/common/arrays", "vs/base/common/objects", "vs/platform/backup/common/backup", "vs/platform/environment/common/environment", "vs/platform/state/common/state", "vs/code/electron-main/window", "vs/platform/environment/node/argv", "electron", "vs/code/node/paths", "vs/platform/lifecycle/electron-main/lifecycleMain", "vs/platform/configuration/common/configuration", "vs/platform/log/common/log", "vs/code/node/windowsFinder", "vs/base/common/event", "vs/platform/product/node/product", "vs/platform/telemetry/common/telemetry", "vs/platform/history/common/history", "vs/base/common/platform", "vs/platform/workspaces/common/workspaces", "vs/platform/instantiation/common/instantiation", "vs/base/common/labels", "vs/base/common/network", "vs/base/common/normalization", "vs/base/common/uri", "vs/base/common/async", "vs/base/node/pfs", "vs/base/common/resources", "vs/platform/remote/common/remoteHosts", "vs/code/electron-main/windowsStateStorage", "vs/platform/workspaces/electron-main/workspacesMainService"], function (require, exports, fs, path_1, nls_1, arrays, objects_1, backup_1, environment_1, state_1, window_1, argv_1, electron_1, paths_1, lifecycleMain_1, configuration_1, log_1, windowsFinder_1, event_1, product_1, telemetry_1, history_1, platform_1, workspaces_1, instantiation_1, labels_1, network_1, normalization_1, uri_1, async_1, pfs_1, resources_1, remoteHosts_1, windowsStateStorage_1, workspacesMainService_1) {
    ...
    let WindowsManager = class WindowsManager {
        constructor(machineId, logService, stateService, environmentService, lifecycleService, backupMainService, telemetryService, configurationService, historyMainService, workspacesMainService, instantiationService) {
        ...
        doExtractPathsFromAPI(openConfig) {
            ...
            const parseOptions = { gotoLineMode: cli && cli.goto, forceOpenWorkspaceAsFile: openConfig.forceOpenWorkspaceAsFile };
            for (const pathToOpen of openConfig.urisToOpen || []) {
                ...
                if (path) {
                    ...
                    let message, detail;
                    if (pathToOpen.uri.scheme === network_1.Schemas.file) {
                        message = nls_1.localize('pathNotExistTitle', "Path does not exist");
                        detail = nls_1.localize('pathNotExistDetail', "The path '{0}' does not seem to exist anymore on disk.", pathToOpen.uri.fsPath);
                    }
                    else {
                        message = nls_1.localize('uriInvalidTitle', "URI can not be opened");
                        detail = nls_1.localize('uriInvalidDetail', "The URI '{0}' is not valid and can not be opened.", pathToOpen.uri.toString());
                    }
                    const options = {
        ...
        parseUri(uri, typeHint, options = {}) {
            ...
            if (!typeHint && (workspaces_1.hasWorkspaceFileExtension(uri.path) || options.gotoLineMode)) {
                typeHint = 'file';
            }
            if (typeHint === 'file') {
                ...
                if (workspaces_1.hasWorkspaceFileExtension(uri.path) && !options.forceOpenWorkspaceAsFile) {
                    return {
        ...
        openExtensionDevelopmentHostWindow(extensionDevelopmentPath, openConfig) {
            ...
            if (cliArgs.length && cliArgs.some(path => !!windowsFinder_1.findWindowOnWorkspaceOrFolderUri(WindowsManager.WINDOWS, uri_1.URI.file(path)))) {
                cliArgs = [];
            }
            if (folderUris.length && folderUris.some(uri => !!windowsFinder_1.findWindowOnWorkspaceOrFolderUri(WindowsManager.WINDOWS, this.argToUri(uri)))) {
                folderUris = [];
            }
            if (fileUris.length && fileUris.some(uri => !!windowsFinder_1.findWindowOnWorkspaceOrFolderUri(WindowsManager.WINDOWS, this.argToUri(uri)))) {
                fileUris = [];
            }
            ...
            if (match) {
                openConfig.cli['remote'] = uri_1.URI.parse(extensionDevelopmentPath).authority;
            }
            // Open it
        ...
        reload(win, cli) {
            ...
            this.lifecycleService.unload(win, 3 /* RELOAD */).then(veto => {
                if (!veto) {
        ...
        doPickAndOpen(options, pickFolders, pickFiles) {
            const internalOptions = options;
            internalOptions.pickFolders = pickFolders;
            internalOptions.pickFiles = pickFiles;
            const dialogOptions = internalOptions.dialogOptions || Object.create(null);
    ...
    class Dialogs {
        constructor(environmentService, telemetryService, stateService, windowsMainService) {
            this.environmentService = environmentService;
            this.telemetryService = telemetryService;
            this.stateService = stateService;
        ...
        pickAndOpen(options) {
            this.getFileOrFolderUris(options).then(paths => {
                ...
                if (options.telemetryEventName) {
                    ...
                    this.telemetryService.publicLog(options.telemetryEventName, Object.assign({}, options.telemetryExtraData, { outcome: numberOfPaths ? 'success' : 'canceled', numberOfPaths }));
                }
                // Open
                if (numberOfPaths) {
        ...
        getFileOrFolderUris(options) {
            ...
            if (!dialogOptions.defaultPath) {
                dialogOptions.defaultPath = this.stateService.getItem(Dialogs.workingDirPickerStorageKey);
            }
            // Ensure properties
            ...
            return this.showOpenDialog(dialogOptions, focusedWindow).then(paths => {
                if (paths && paths.length > 0) {
                    ...
                    for (const path of paths) {
                        result.push({ uri: uri_1.URI.file(path) });
                    }
                    return result;
                }
                return undefined;
        ...
        showMessageBox(options, window) {
            return this.getDialogQueue(window).queue(() => {
                return new Promise(resolve => {
                    electron_1.dialog.showMessageBox(window ? window.win : undefined, options, (response, checkboxChecked) => {
        ...
        showSaveDialog(options, window) {
            ...
            return this.getDialogQueue(window).queue(() => {
                return new Promise(resolve => {
                    electron_1.dialog.showSaveDialog(window ? window.win : undefined, options, path => {
            ...
            return this.getDialogQueue(window).queue(() => {
                return new Promise(resolve => {
                    ...
                    validatePathPromise.then(() => {
                        electron_1.dialog.showOpenDialog(window ? window.win : undefined, options, paths => {
    ...
    class WorkspacesManager {
        ...
        enterWorkspace(window, path) {
            ...
            return this.isValidTargetWorkspacePath(window, path).then(isValid => {
                ...
                const workspaceIdentifier = workspacesMainService_1.getWorkspaceIdentifier(path);
