    ...
    suite('Workbench - TerminalEnvironment', () => {
        ...
        suite('getCwd', () => {
            ...
            test('should use to the workspace if it exists', () => {
                assertPathsMatch(terminalEnvironment.getCwd({ executable: undefined, args: [] }, '/userHome/', uri_1.URI.file('/foo'), undefined), '/foo');
            });
            ...
            test('should normalize a relative custom cwd against the workspace path', () => {
                assertPathsMatch(terminalEnvironment.getCwd({ executable: undefined, args: [] }, '/userHome/', uri_1.URI.file('/bar'), 'foo'), '/bar/foo');
                assertPathsMatch(terminalEnvironment.getCwd({ executable: undefined, args: [] }, '/userHome/', uri_1.URI.file('/bar'), './foo'), '/bar/foo');
                assertPathsMatch(terminalEnvironment.getCwd({ executable: undefined, args: [] }, '/userHome/', uri_1.URI.file('/bar'), '../foo'), '/foo');
            });
            ...
            test('should ignore custom cwd when told to ignore', () => {
                assertPathsMatch(terminalEnvironment.getCwd({ executable: undefined, args: [], ignoreConfigurationCwd: true }, '/userHome/', uri_1.URI.file('/bar'), '/foo'), '/bar');
            });
        });
    });
});
