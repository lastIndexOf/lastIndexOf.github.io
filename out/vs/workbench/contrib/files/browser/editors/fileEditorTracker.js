    ...
    let FileEditorTracker = class FileEditorTracker extends lifecycle_2.Disposable {
        ...
        onWindowFocusChange(focused) {
            if (focused) {
                ...
                arrays_1.distinct(arrays_1.coalesce(this.editorService.visibleEditors
                    .map(editorInput => {
                    const resource = editor_1.toResource(editorInput, { supportSideBySide: true });
        ...
        handleDeletes(arg1, isExternal, movedTo) {
            ...
            nonDirtyFileEditors.forEach(editor => {
                ...
                if (this.closeOnFileDelete || !isExternal || !editor.isResolved()) {
                    ...
                    let checkExists;
                    if (isExternal) {
                        checkExists = async_1.timeout(100).then(() => this.fileService.existsFile(resource));
                    }
                    else {
        ...
        handleUpdatesToVisibleBinaryEditors(e) {
            ...
            editors.forEach(editor => {
                const resource = editor.input ? editor_1.toResource(editor.input, { supportSideBySide: true }) : undefined;
