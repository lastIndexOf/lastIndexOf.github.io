    ...
    let SelectColorThemeAction = class SelectColorThemeAction extends actions_1.Action {
        ...
        run() {
            return this.themeService.getColorThemes().then(themes => {
                ...
                const picks = [].concat(toEntries(themes.filter(t => t.type === themeService_1.LIGHT), nls_1.localize('themes.category.light', "light themes")), toEntries(themes.filter(t => t.type === themeService_1.DARK), nls_1.localize('themes.category.dark', "dark themes")), toEntries(themes.filter(t => t.type === themeService_1.HIGH_CONTRAST), nls_1.localize('themes.category.hc', "high contrast themes")), configurationEntries(this.extensionGalleryService, nls_1.localize('installColorThemes', "Install Additional Color Themes...")));
                const selectTheme = (theme, applyTheme) => {
                    ...
                    this.themeService.setColorTheme(theme.id, target).then(undefined, err => {
                ...
                const autoFocusIndex = arrays_1.firstIndex(picks, p => p.type !== 'separator' && p.id === currentTheme.id);
        ...
        run() {
            return this.themeService.getFileIconThemes().then(themes => {
                ...
                const selectTheme = (theme, applyTheme) => {
                    ...
                    this.themeService.setFileIconTheme(theme && theme.id, target).then(undefined, err => {
                        errors_1.onUnexpectedError(err);
