 ...
 *--------------------------------------------------------------------------------------------*/
define(["require", "exports", "assert", "vs/workbench/contrib/externalTerminal/electron-browser/externalTerminalService", "vs/workbench/contrib/externalTerminal/electron-browser/externalTerminal"], function (require, exports, assert_1, externalTerminalService_1, externalTerminal_1) {
    "use strict";
    ...
    suite('ExternalTerminalService', () => {
        ...
        test(`WinTerminalService - uses default terminal when configuration.terminal.external.windowsExec is undefined`, done => {
            ...
            let mockSpawner = {
                spawn: (command, args, opts) => {
                    // assert
                    assert_1.equal(args[args.length - 1], externalTerminal_1.getDefaultTerminalWindows(), 'terminal should equal expected');
                    done();
                    return {
                        on: (evt) => evt
                    };
        ...
        test(`MacTerminalService - uses default terminal when configuration.terminal.external.osxExec is undefined`, done => {
            ...
            let mockSpawner = {
                spawn: (command, args, opts) => {
                    // assert
                    assert_1.equal(args[1], externalTerminal_1.DEFAULT_TERMINAL_OSX, 'terminal should equal expected');
                    done();
                    return {
                        on: (evt) => evt
                    };
        ...
        test(`LinuxTerminalService - uses default terminal when configuration.terminal.external.linuxExec is undefined`, done => {
            externalTerminal_1.getDefaultTerminalLinuxReady().then(defaultTerminalLinux => {
                let testCwd = 'path/to/workspace';
                let mockSpawner = {
                    spawn: (command, args, opts) => {
                        // assert
