    ...
    class MarkerWidget extends lifecycle_1.Disposable {
        constructor(parent, markersViewModel, instantiationService) {
            ...
            this.actionBar = this._register(new actionbar_1.ActionBar(dom.append(parent, dom.$('.actions')), {
                actionItemProvider: (action) => action.id === markersPanelActions_1.QuickFixAction.ID ? instantiationService.createInstance(markersPanelActions_1.QuickFixActionItem, action) : null
            }));
            ...
            this._register(lifecycle_1.toDisposable(() => this.disposables = lifecycle_1.dispose(this.disposables)));
        }
        ...
        renderQuickfixActionbar(marker) {
            ...
            if (viewModel) {
                ...
                quickFixAction.onShowQuickFixes(() => {
                    const quickFixActionItem = this.actionBar.items[0];
        ...
        renderMessageAndDetails(element, filterData) {
            ...
            for (let index = 0; index < lines.length; index++) {
                ...
                const highlightedLabel = new highlightedLabel_1.HighlightedLabel(lastLineElement, false);
        ...
        renderDetails(marker, filterData, parent) {
            ...
            const source = new highlightedLabel_1.HighlightedLabel(dom.append(parent, dom.$('')), false);
            source.set(marker.source, sourceMatches);
            dom.toggleClass(source.element, 'marker-source', !!marker.source);
            const code = new highlightedLabel_1.HighlightedLabel(dom.append(parent, dom.$('')), false);
        ...
        static iconClassNameFor(element) {
                ...
                if (model) {
                    if (!this.codeActionsPromise) {
                        this.codeActionsPromise = async_1.createCancelablePromise(cancellationToken => {
                            return codeAction_1.getCodeActions(model, new range_1.Range(this.marker.range.startLineNumber, this.marker.range.startColumn, this.marker.range.endLineNumber, this.marker.range.endColumn), { type: 'manual', filter: { kind: codeActionTrigger_1.CodeActionKind.QuickFix } }, cancellationToken);
                        });
                    }
                    return this.codeActionsPromise;
                }
        ...
        toActions(codeActions) {
            return codeActions.actions.map(codeAction => new actions_1.Action(codeAction.command ? codeAction.command.id : codeAction.title, codeAction.title, undefined, true, () => {
                return this.openFileAtMarker(this.marker)
                    .then(() => codeActionCommands_1.applyCodeAction(codeAction, this.bulkEditService, this.commandService));
            }));
        }
