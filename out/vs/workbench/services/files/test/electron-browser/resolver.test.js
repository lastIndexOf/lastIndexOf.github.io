 ...
 *--------------------------------------------------------------------------------------------*/
define(["require", "exports", "fs", "vs/base/common/path", "assert", "vs/workbench/services/files/node/fileService", "vs/base/common/uri", "vs/base/common/platform", "vs/workbench/services/files/test/electron-browser/utils", "vs/base/common/amd"], function (require, exports, fs, path, assert, fileService_1, uri_1, platform_1, utils, amd_1) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    function create(relativePath) {
        let basePath = amd_1.getPathFromAmdModule(require, './fixtures/resolver');
        let absolutePath = relativePath ? path.join(basePath, relativePath) : basePath;
        let fsStat = fs.statSync(absolutePath);
        return new fileService_1.StatResolver(uri_1.URI.file(absolutePath), fsStat.isSymbolicLink(), fsStat.isDirectory(), fsStat.mtime.getTime(), fsStat.size, undefined);
    }
    function toResource(relativePath) {
        let basePath = amd_1.getPathFromAmdModule(require, './fixtures/resolver');
        let absolutePath = relativePath ? path.join(basePath, relativePath) : basePath;
        return uri_1.URI.file(absolutePath);
    }
    suite('Stat Resolver', () => {
        ...
        test('resolve directory - resolveTo single directory - mixed casing', function () {
            ...
            return resolver.resolve({ resolveTo: [toResource('other/Deep')] }).then(result => {
                ...
                assert.ok(other.children.length > 0);
                const deep = utils.getByName(other, 'deep');
                if (platform_1.isLinux) { // Linux has case sensitive file system
                    assert.ok(deep);
