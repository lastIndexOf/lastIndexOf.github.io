...
define(["require", "exports", "vs/base/common/async", "vs/base/common/cancellation", "vs/base/common/errors", "vs/base/common/event", "vs/base/common/lifecycle", "vs/base/common/uriIpc", "vs/workbench/services/extensions/node/lazyPromise", "vs/workbench/services/extensions/node/proxyIdentifier"], function (require, exports, async_1, cancellation_1, errors, event_1, lifecycle_1, uriIpc_1, lazyPromise_1, proxyIdentifier_1) {
    ...
    class MessageBuffer {
        ...
        readUInt8() {
            const n = this._buff.readUInt8(this._offset, true);
            this._offset += 1;
            return n;
        }
        writeUInt32(n) {
        ...
        readUInt32() {
            const n = this._buff.readUInt32BE(this._offset, true);
            this._offset += 4;
            return n;
        }
        static sizeShortString(str, strByteLength) {
            return 1 /* string length */ + strByteLength /* actual string */;
        }
        writeShortString(str, strByteLength) {
        ...
        readShortString() {
            const strLength = this._buff.readUInt8(this._offset, true);
            this._offset += 1;
            const str = this._buff.toString('utf8', this._offset, this._offset + strLength);
            this._offset += strLength;
            return str;
        }
        static sizeLongString(str, strByteLength) {
            return 4 /* string length */ + strByteLength /* actual string */;
        }
        writeLongString(str, strByteLength) {
        ...
        readLongString() {
            const strLength = this._buff.readUInt32LE(this._offset, true);
            this._offset += 4;
            const str = this._buff.toString('utf8', this._offset, this._offset + strLength);
            this._offset += strLength;
            return str;
        }
        static sizeBuffer(buff, buffByteLength) {
            return 4 /* buffer length */ + buffByteLength /* actual buffer */;
        }
        writeBuffer(buff, buffByteLength) {
        ...
        readBuffer() {
            const buffLength = this._buff.readUInt32LE(this._offset, true);
            this._offset += 4;
            const buff = this._buff.slice(this._offset, this._offset + buffLength);
            this._offset += buffLength;
            return buff;
        }
        static sizeMixedArray(arr, arrLengths) {
            let size = 0;
            size += 1; // arr length
        ...
        writeMixedArray(arr, arrLengths) {
        ...
        readMixedArray() {
            const arrLen = this._buff.readUInt8(this._offset, true);
            this._offset += 1;
    ...
    class MessageIO {
        ...
        static serializeRequest(req, rpcId, method, args, usesCancellationToken, replacer) {
            if (this._arrayContainsBuffer(args)) {
                let massagedArgs = new Array(args.length);
                let argsLengths = new Array(args.length);
        ...
        static _requestMixedArgs(req, rpcId, method, args, argsLengths, usesCancellationToken) {
            const methodByteLength = Buffer.byteLength(method, 'utf8');
            let len = 0;
            len += MessageBuffer.sizeUInt8();
            len += MessageBuffer.sizeShortString(method, methodByteLength);
        ...
        static _serializeReplyOKBuffer(req, res) {
            ...
            let result = MessageBuffer.alloc(8 /* ReplyOKBuffer */, req, len);
