...
define(["require", "exports", "vs/base/common/lifecycle", "vs/base/common/event", "vs/base/common/types", "vs/base/browser/dom", "vs/base/common/numbers", "vs/base/common/arrays", "vs/base/browser/ui/sash/sash", "vs/base/common/color", "vs/base/browser/event", "vs/base/browser/ui/sash/sash", "vs/css!./splitview"], function (require, exports, lifecycle_1, event_1, types, dom, numbers_1, arrays_1, sash_1, color_1, event_2, sash_2) {
    ...
    class SplitView extends lifecycle_1.Disposable {
        ...
        addView(view, size, index = this.viewItems.length) {
            ...
            const layoutContainer = this.orientation === 0 /* VERTICAL */
            ...
            if (this.viewItems.length > 1) {
                const orientation = this.orientation === 0 /* VERTICAL */ ? 1 /* HORIZONTAL */ : 0 /* VERTICAL */;
                const layoutProvider = this.orientation === 0 /* VERTICAL */ ? { getHorizontalSashTop: (sash) => this.getSashPosition(sash) } : { getVerticalSashLeft: (sash) => this.getSashPosition(sash) };
                const sash = new sash_1.Sash(this.sashContainer, layoutProvider, {
                    orientation,
                ...
                const onDidResetDisposable = sash.onDidReset(() => this._onDidSashReset.fire(arrays_1.firstIndex(this.sashItems, item => item.sash === sash)));
        ...
        layout(size) {
            ...
            if (!this.proportions) {
                ...
                const lowPriorityIndexes = indexes.filter(i => this.viewItems[i].view.priority === 1 /* Low */);
                const highPriorityIndexes = indexes.filter(i => this.viewItems[i].view.priority === 2 /* High */);
        ...
        onSashStart({ sash, start, alt }) {
            ...
            const disposable = lifecycle_1.combinedDisposable([
                event_2.domEvent(document.body, 'keydown')(e => resetSashDragState(this.sashDragState.current, e.altKey)),
                event_2.domEvent(document.body, 'keyup')(() => resetSashDragState(this.sashDragState.current, false))
            ]);
        ...
        resizeView(index, size) {
            ...
            if (delta !== 0 && index < this.viewItems.length - 1) {
                ...
                const deltaDown = numbers_1.clamp(delta, -expandDown, collapseDown);
                this.resize(index, deltaDown);
                delta -= deltaDown;
            }
            if (delta !== 0 && index > 0) {
                const upIndexes = arrays_1.range(index - 1, -1);
                ...
                const deltaUp = numbers_1.clamp(-delta, -collapseUp, expandUp);
        ...
        resize(index, delta, sizes = this.viewItems.map(i => i.size), lowPriorityIndexes, highPriorityIndexes, overloadMinDelta = Number.NEGATIVE_INFINITY, overloadMaxDelta = Number.POSITIVE_INFINITY) {
            ...
            for (let i = 0, deltaUp = delta; i < upItems.length; i++) {
                ...
                const size = numbers_1.clamp(upSizes[i] + deltaUp, item.view.minimumSize, item.view.maximumSize);
            ...
            for (let i = 0, deltaDown = delta; i < downItems.length; i++) {
                ...
                const size = numbers_1.clamp(downSizes[i] - deltaDown, item.view.minimumSize, item.view.maximumSize);
        ...
        distributeEmptySpace() {
            ...
            for (let i = this.viewItems.length - 1; emptyDelta !== 0 && i >= 0; i--) {
                const item = this.viewItems[i];
                const size = numbers_1.clamp(item.size + emptyDelta, item.view.minimumSize, item.view.maximumSize);
