 ...
 *--------------------------------------------------------------------------------------------*/
define(["require", "exports", "vs/base/browser/browser", "vs/base/common/event", "vs/base/common/lifecycle", "vs/base/common/platform", "vs/editor/browser/config/charWidthReader", "vs/editor/browser/config/elementSizeObserver", "vs/editor/common/config/commonEditorConfig", "vs/editor/common/config/fontInfo"], function (require, exports, browser, event_1, lifecycle_1, platform, charWidthReader_1, elementSizeObserver_1, commonEditorConfig_1, fontInfo_1) {
    "use strict";
    ...
    function restoreFontInfo(storageService) {
        const strStoredFontInfo = storageService.get('editorFontInfo', 0 /* GLOBAL */);
        if (typeof strStoredFontInfo !== 'string') {
            return;
        }
        let storedFontInfo = null;
        try {
    ...
    class CSSBasedConfiguration extends lifecycle_1.Disposable {
        ...
        restoreFontInfo(savedFontInfo) {
            // Take all the saved font info and insert them in the cache without the trusted flag.
            // The reason for this is that a font might have been installed on the OS in the meantime.
            for (let i = 0, len = savedFontInfo.length; i < len; i++) {
                const fontInfo = new fontInfo_1.FontInfo(savedFontInfo[i], false);
    ...
    class Configuration extends commonEditorConfig_1.CommonEditorConfiguration {
        constructor(options, referenceDomElement = null, accessibilityService) {
            super(options);
            this.accessibilityService = accessibilityService;
            this._elementSizeObserver = this._register(new elementSizeObserver_1.ElementSizeObserver(referenceDomElement, () => this._onReferenceDomElementSizeChanged()));
